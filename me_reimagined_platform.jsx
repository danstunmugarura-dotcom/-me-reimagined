...
const WEEKS = [
  ...
  { id:4,  driver:2, title:"Root Cause & Strength",         phase:"D2 Diagnose · Part Two",    videos:1, scorecard:60,  progress:0,   locked:false },
  { id:5,  driver:3, title:"Time, Tasks & Priorities",      phase:"D3 Discuss · Part One",     videos:1, scorecard:50,  progress:0,   locked:false },
  { id:6,  driver:3, title:"People, Process & the 4S",      phase:"D3 Discuss · Part Two",     videos:1, scorecard:50,  progress:0,   locked:false },
  { id:7,  driver:4, title:"Structure & Standards",         phase:"D4 Design · Part One",      videos:3, scorecard:120, progress:0,   locked:false },
  { id:8,  driver:4, title:"Systems & Simplicity",          phase:"D4 Design · Part Two",      videos:2, scorecard:60,  progress:0,   locked:false },
  { id:9,  driver:4, title:"Your Complete Operating System",phase:"D4 Design · Integration",   videos:1, scorecard:50,  progress:0,   locked:false },
  { id:10, driver:5, title:"Embody",                        phase:"D5 Deliver · Part One",     videos:1, scorecard:50,  progress:0,   locked:false },
  { id:11, driver:5, title:"Execute & Evaluate",            phase:"D5 Deliver · Part Two",     videos:1, scorecard:100, progress:0,   locked:false },
  { id:12, driver:5, title:"Evolve & Final Integration",    phase:"D5 Deliver · The New You",  videos:1, scorecard:200, progress:0,   locked:false },
];
...
// ─── FREE ACCESS ENTRY ────────────────────────────────────
function FreeEntry({ onEnter }) {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const inp = { width:"100%", background:"#1B2E45", border:`1px solid ${C.border}`, borderRadius:4, padding:"12px 14px", color:C.white, fontSize:14, fontFamily:"Arial,sans-serif", boxSizing:"border-box", outline:"none" };

  return (
    <div style={{ background:C.navy, minHeight:"100vh", color:C.white, fontFamily:"Georgia,serif", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ maxWidth:400, width:"100%", padding:"0 24px" }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <Logo size={24}/>
          <div style={{ marginTop:24, fontSize:26 }}>Welcome.</div>
          <div style={{ fontSize:14, color:C.muted, fontFamily:"Arial,sans-serif", marginTop:8, lineHeight:1.6 }}>
            You have been given complimentary access to Me Reimagined. Enter your details to begin.
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            <div style={{ fontSize:10, letterSpacing:1.5, color:C.muted, fontFamily:"Arial,sans-serif", marginBottom:6 }}>YOUR NAME</div>
            <input style={inp} placeholder="First and last name" value={name} onChange={e=>setName(e.target.value)}/>
          </div>
          <div>
            <div style={{ fontSize:10, letterSpacing:1.5, color:C.muted, fontFamily:"Arial,sans-serif", marginBottom:6 }}>EMAIL ADDRESS</div>
            <input style={inp} placeholder="you@email.com" value={email} onChange={e=>setEmail(e.target.value)}/>
          </div>
          <button
            onClick={()=>name.trim()&&email.trim()&&onEnter(name.trim())}
            disabled={!name.trim()||!email.trim()}
            style={{ background:name.trim()&&email.trim()?C.gold:C.mutedDark, border:"none", color:C.navy, padding:"14px", borderRadius:4, cursor:name.trim()&&email.trim()?"pointer":"not-allowed", fontSize:14, fontWeight:"bold", fontFamily:"Arial,sans-serif", marginTop:4 }}>
            Enter Me Reimagined →
          </button>
          <div style={{ textAlign:"center", fontSize:11, color:C.mutedDark, fontFamily:"Arial,sans-serif" }}>
            Complimentary access · Code1 Beta Programme
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{ background:C.navy, minHeight:"100vh", color:C.white, fontFamily:"Georgia,serif" }}>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"32px 24px 64px" }}>
          ...
          {currentSection.questions.map((q, qi)=>{
            ...
            return (
              <div key={qi} style={{ marginBottom:20 }}>
                ...
                <textarea value={answers[key]||""} onChange={e=>setAns(key,e.target.value)}
                  style={{ width:"100%", background:"#0D1B2A", border:`1px solid ${answers[key]?.trim()?C.gold:C.border}`, borderRadius:4, padding:"10px 12px", color:C.white, fontSize:13, fontFamily:"Arial,sans-serif", resize:"vertical", minHeight:72, outline:"none", boxSizing:"border-box" }}
        ...
        <div style={{ display:"flex", justifyContent:"space-between", gap:12 }}>
          ...
          {isLast ? (
            <button onClick={onComplete} style={{ background:C.gold, border:"none", color:C.navy, padding:"11px 28px", borderRadius:4, cursor:"pointer", fontSize:14, fontWeight:"bold", fontFamily:"Arial,sans-serif" }}>Complete iPhilosophy & Begin Week 1 →</button>
          ) : (
            <button onClick={()=>setSection(section+1)} style={{ background:C.gold, border:"none", color:C.navy, padding:"11px 28px", borderRadius:4, cursor:"pointer", fontSize:14, fontWeight:"bold", fontFamily:"Arial,sans-serif" }}>Next Section →</button>
          )}
        </div>
      </div>
    </div>
...
// ─── DASHBOARD ────────────────────────────────────────────
function Dashboard({ name, onSelectWeek, onSignOut, onMonthly }) {
    ...
    <div style={{ background:C.navy, minHeight:"100vh", color:C.white, fontFamily:"Georgia,serif" }}>
      <nav style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 22px", borderBottom:`1px solid ${C.border}`, background:C.navyMid, position:"sticky", top:0, zIndex:10 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          <div style={{ fontSize:12, color:C.muted, fontFamily:"Arial,sans-serif" }}>Welcome, <span style={{ color:C.gold }}>{name}</span></div>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px 48px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:20, marginBottom:28, alignItems:"start" }}>
          <div>
            ...
            <div style={{ fontSize:13, color:C.muted, fontFamily:"Arial,sans-serif" }}>All weeks are unlocked. Begin your journey.</div>
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10, marginBottom:28 }}>
          {[
            ...
            { l:"Driver",     v:"D1 Discover",          c:C.accent},
          ].map(s=>(
            ...
            <div key={di} style={{ marginBottom:28 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {driverWeeks.map(week=>(
                  <button key={week.id} onClick={()=>onSelectWeek(week.id)}
                    style={{ background:C.navyMid, border:`1px solid ${week.progress>0&&week.progress<100?driver.color+"66":C.border}`, borderRadius:4, padding:"12px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:12, textAlign:"left", width:"100%", transition:"border-color 0.2s" }}>
                    ...
                    <span style={{ color:C.mutedDark, fontSize:18, flexShrink:0 }}>›</span>
                  </button>
                ))}
              </div>
            </div>
  ...
  return (
    <div style={{ background:C.navy, minHeight:"100vh", color:C.white, fontFamily:"Georgia,serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        <div style={{ width:220, flexShrink:0, background:"#0a1520", borderRight:`1px solid ${C.border}`, overflowY:"auto" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"22px 26px" }}>
          ...
          {cur.type==="video" && (
            <div>
              <div style={{ background:C.navyMid, borderRadius:6, aspectRatio:"16/9", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:18, border:`1px solid ${C.border}`, position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ position:"relative", textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:12, color:driver.color, fontFamily:"Arial,sans-serif", marginTop:4 }}>Video coming 2 July 2026</div>
                </div>
              </div>
          ...
          {cur.type==="exercise" && (
            <div>
              <div style={{ fontSize:14, color:C.muted, fontFamily:"Arial,sans-serif", lineHeight:1.6, marginBottom:20 }}>
                These exercises are the work of {driver.name}. Honest answers only.
              </div>
              {["What are you discovering about yourself in this driver?","Where is the resistance coming from?","What would the reimagined version of you do differently starting today?"].map((q,i)=>(
          ...
          {cur.type==="scorecard" && (
            <div>
              <div style={{ fontSize:13, color:C.muted, fontFamily:"Arial,sans-serif", marginBottom:16 }}>Rate each statement 1–10. Rate what is actually true right now.</div>
              <div style={{ background:C.navyMid, border:`1px solid ${C.border}`, borderRadius:4, overflow:"hidden", marginBottom:16 }}>
                <div style={{ background:C.navyLight, padding:"10px 16px", borderBottom:`1px solid ${C.border}` }}>
                ...
                {["I have a clear picture of where I am in this area","I understand what is driving my current results","I can name the patterns that keep appearing","I know what needs to change and why","I am ready to take action on what this reveals"].map((s,i)=>(
                  <div key={i} style={{ padding:"11px 16px", borderBottom:i<4?`1px solid ${C.border}`:"none", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12 }}>
                    ...
                    <div style={{ fontSize:14, color:C.gold, fontFamily:"Arial,sans-serif", fontWeight:"bold", background:"#0D1B2A", padding:"4px 10px", borderRadius:3, minWidth:32, textAlign:"center" }}>___</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {cur.type==="ailab" && (
            <div>
              <div style={{ background:`${driver.color}11`, border:`1px solid ${driver.color}44`, borderRadius:4, padding:"16px 18px", marginBottom:18 }}>
                ...
                <div style={{ fontSize:13, color:C.muted, fontFamily:"Arial,sans-serif", lineHeight:1.6, marginBottom:14 }}>Human Intelligence leads. AI supports. Use the prompt below with any AI assistant.</div>
          ...
          {cur.type==="commit" && (
            <div>
              <div style={{ fontSize:14, color:C.muted, fontFamily:"Arial,sans-serif", lineHeight:1.6, marginBottom:22 }}>Write your commitments with precision. Vague intentions produce vague results.</div>
...
export default function App() {
  ...
  if (view==="entry")      return <FreeEntry onEnter={name=>{setParticipantName(name);setView("iphilosophy");}}/>;
  ...
  return <Dashboard name={participantName} onSelectWeek={id=>{setSelectedWeek(id);setView("week");}} onSignOut={()=>setView("entry")} onMonthly={()=>setView("live")}/>;
}
